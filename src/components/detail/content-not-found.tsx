...
export function ContentNotFound({ type, slug, className }: ContentNotFoundProps) {
    ...
    <main className={cn('min-h-screen bg-background', className)} role="main">
      <div className="container mx-auto px-4 py-16">
        <div className="max-w-2xl mx-auto text-center space-y-8">
          ...
          {/* Suggestions */}
          <section className="bg-muted/30 rounded-lg p-6 text-left space-y-4" aria-labelledby="suggestions-heading">
            <h2 id="suggestions-heading" className="font-semibold text-lg">What you can do:</h2>
            <ul className="space-y-2 text-muted-foreground" role="list">
              ...
              <li className="flex items-start gap-2" role="listitem">
                <span className="w-1.5 h-1.5 bg-primary rounded-full mt-2 flex-shrink-0" aria-hidden="true" />
                <span>Use the search functionality to find related content</span>
              </li>
              <li className="flex items-start gap-2" role="listitem">
                <span className="w-1.5 h-1.5 bg-primary rounded-full mt-2 flex-shrink-0" aria-hidden="true" />
...
export function ProjectNotFound({ slug, className }: { slug?: string; className?: string }) {
  return <ContentNotFound type="project" slug={slug} className={className} />;
}

export function WritingNotFound({ slug, className }: { slug?: string; className?: string }) {
