...
export async function getContentWithMetadata(
  ...
  slug: string
): Promise<(EnhancedProject | EnhancedWriting) & { seoMetadata: ReturnType<typeof generateSEOMetadata> } | null> {
...
export async function getFilteredContent<T extends EnhancedProject | EnhancedWriting>(
  type: 'projects' | 'writings',
  filters: {
    tags?: string[];
    featured?: boolean;
  ...
  if (filters.featured !== undefined) {
    filteredContent = filteredContent.filter(item =>
      item.frontmatter.featured === filters.featured
    );
  }
  ...
  if (type === 'projects' && filters.status) {
    filteredContent = filteredContent.filter(item =>
      (item as EnhancedProject).frontmatter.status === filters.status
    );
  }
...
export async function getContentStats(): Promise<{
  ...
  const projectStats = {
    ...
    completed: allProjects.filter(p => p.frontmatter.status === 'completed').length,
    inProgress: allProjects.filter(p => p.frontmatter.status === 'in-progress').length,
    planned: allProjects.filter(p => p.frontmatter.status === 'planned').length,
  };
  ...
  allWritings.forEach(writing => {
    if (writing.frontmatter.category) {
      categories[writing.frontmatter.category] = (categories[writing.frontmatter.category] || 0) + 1;
    }
  });
...
export async function searchContent(
  ...
  const searchInContent = (item: EnhancedProject | EnhancedWriting): boolean => {
